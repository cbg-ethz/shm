...
class HLM:
    def __init__(data: pd.DataFrame,
        ...
        conditions = sp.unique(data["Condition"].values)
        genes = sp.unique(data["Gene"].values)
        gene_idx = le.fit_transform(data["Gene"].values)
        con_idx = le.fit_transform(data["Condition"].values)

        len_genes = len(sp.unique(gene_idx))
        len_conditions = len(sp.unique(con_idx))
        len_sirnas = len(sp.unique(data["sgRNA"].values))
        len_replicates = len(sp.unique(data["replicate"].values))
        ...
        gene_conds = ["{}-{}".format(a, b) for a, b in
